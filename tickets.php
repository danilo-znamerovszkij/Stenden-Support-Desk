...
{ //updating the db
    if ($_SESSION['userType'] == "Team Leader" && isset($_POST['assign']))
    {
        $id = $_POST['id'];
...
{ //updating the db
    $id = $_POST['id_del'];
    $assignQuery = "Delete from incident WHERE incident.incident_id =  '$id'";
    mysqli_query($conn, $assignQuery);
}
...
$ticketsAll2 = mysqli_query($conn, $sql2);
...
<?= generateMenu() ?>
                ...
                <div class="content">
                        <?php if ($_SESSION['userType'] != "Security Operator")
                        { ?>
...
<?php } ?>
                    ...
                    <?php } else if ($_SESSION['userType'] != "Security Operator")
                    { ?>
                            ...
                            while ($row = mysqli_fetch_assoc($tickets))
                            {
                                ...
                                <tr>
                                    ...
                                    <td><form action="tickets.php" method="post">
                                            ...
                                            <?php if ($_SESSION['userType'] == "Team Leader")
                                            { ?> <!-- team leader view -->
...
{ ?>
    ...
    <?php } ?>
                                                ...
                                                <?php
                                                while ($row1 = mysqli_fetch_assoc($ticketsAll))
                                                {
...
<?php } ?>
                                        ...
                                        <!-- another table Start -->
                                                ...
                                                <?php if ($tickets->num_rows != 0 && $_SESSION['userType'] == 'Security Operator')
                                                { ?>

                                            <table class="fancy-table">
                                                <div class="titleBox"><p>All tickets</p></div>
                                                <tr>
                                                    <th>ID</th>
                                                    <th>Client Name</th>
                                                    <th>Date Submitted</th>
    ...
    <?php
    while ($row = mysqli_fetch_assoc($ticketsAll2))
    {
        $userQuery = mysqli_query($conn, $userString);
        ?>
                                                    <tr>
                                                        ...
                                                        <td><?= $row['status_name'] ?></td>
                                                       <td><form action="tickets.php" method="post">
                                                                <input type="hidden" name="id_del" value="<?= $row['incident_id'] ?>">
